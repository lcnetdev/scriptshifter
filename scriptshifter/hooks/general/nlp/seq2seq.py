...
def tokenize_example(
        example, en_nlp, de_nlp, max_length, lower, sos_token, eos_token):
...
def get_collate_fn(pad_index):
    def collate_fn(batch):
        ...
        batch_en_ids = nn.utils.rnn.pad_sequence(
                batch_en_ids, padding_value=pad_index)
        batch_de_ids = nn.utils.rnn.pad_sequence(
                batch_de_ids, padding_value=pad_index)
        batch = {
            "en_ids": batch_en_ids,
            "de_ids": batch_de_ids,
        }
...
class Encoder(nn.Module):
    def __init__(
            self, input_dim, embedding_dim, hidden_dim, n_layers, dropout):
        super().__init__()
...
class Decoder(nn.Module):
    def __init__(
            self, output_dim, embedding_dim, hidden_dim, n_layers, dropout):
        super().__init__()
    ...
    def forward(self, input, hidden, cell):
        ...
        # hidden = [n layers * n directions, batch size, hidden dim]
        # cell = [n layers * n directions, batch size, hidden dim]
        # seq length and n directions will always be 1 in this decoder,
        # therefore:
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, src, trg, teacher_forcing_ratio):
        ...
        outputs = torch.zeros(
                trg_length, batch_size, trg_vocab_size).to(self.device)
        ...
        for t in range(1, trg_length):
            ...
            # states receive output tensor (predictions) and new hidden and
            # cell states
...
for epoch in tqdm.tqdm(range(n_epochs)):
    ...
    print(
            ...
            f"Train PPL: {np.exp(train_loss):7.3f}")
    print(
            f"\tValid Loss: {valid_loss:7.3f} | "
            f"Valid PPL: {np.exp(valid_loss):7.3f}")
